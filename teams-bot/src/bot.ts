...
function shortFile(sourceFile: string | null | undefined): string {
  if (!sourceFile) return ''
  // Берём только имя файла (после последнего слэша)
...
function formatFileStats(fileName: string, senderName: string, res: any): string {
  ...
  if (r5 > 0 && topSigs.length > 0) {
    ...
    topSigs.forEach((s, i) => {
      ...
      const fln = shortFile(s.sourceFile)
      lines.push(``)
  ...
  if (errors > 0 && errorEvents.length > 0) {
    lines.push(`**⚠️ Errors (${errors}):**`) // без пустой строки перед списком

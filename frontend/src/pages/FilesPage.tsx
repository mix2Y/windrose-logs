    ...
    <div>
      ...
      <div style={{ padding: '10px 28px', borderBottom: '1px solid var(--border)', background: 'var(--bg)', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        {STATUS_OPTIONS.map(s => (
          <button key={s}
            onClick={() => { setStatus(s); setPage(1); loadFiles(1, dateFrom, dateTo, s) }}
            style={{
              fontSize: 11, padding: '3px 10px', borderRadius: 20, cursor: 'pointer', fontWeight: 500,
              border: status === s ? '1.5px solid var(--amber)' : '1.5px solid var(--border)',
              background: status === s ? 'var(--amber-bg)' : 'transparent',
              color: status === s ? 'var(--amber)' : 'var(--text-3)',
              transition: 'all 0.1s',
            }}>
            {s === '' ? 'All' : s}
          </button>

...
export function LogViewerPage() {
  ...
  const [loadingAll, setLoadingAll]   = useState(false)  // loading remaining pages for search
  ...
  const allLinesRef = useRef<{ lineNumber: number; text: string }[]>([])  // always up to date

  // Keep ref in sync with state
  useEffect(() => { allLinesRef.current = lines }, [lines])
  ...
  // ── Load all remaining pages (for search) ───────────────────────────────
  const loadAllPages = useCallback(async (currentPage: number, total: number) => {
    ...
    try {
      ...
      setLines(prev => [...prev, ...allNew])
      setPage(total)
    } catch { /**/ }
    finally { setLoadingAll(false) }
  }, [id])

  // ── Infinite scroll (manual browsing, no search) ─────────────────────────
  const loadMore = useCallback(() => {
    if (searchTerm) return  // don't lazy-load while searching (loadAllPages handles it)
  ...
  // ── When search term set: load all pages first, then search ──────────────
  useEffect(() => {
    if (!searchTerm) { setMatchIndices([]); setCurrentMatch(0); return }
    // Load remaining pages if needed
    if (page < totalPages && !loadingAll) {
      loadAllPages(page, totalPages).then(() => {
        // search runs again via lines dependency after load
      })
    }
  }, [searchTerm])  // eslint-disable-line

  // ── Recompute matches whenever lines or searchTerm change ────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const lineIndex = matchIndices[currentMatch]
    const lineNum = lines[lineIndex]?.lineNumber
  ...
  function handleSearchInput(v: string) {
    ...
    searchTimer.current = setTimeout(() => setSearchTerm(v.trim()), 300)
  }
  function clearSearch() { setInputVal(''); setSearchTerm(''); setMatchIndices([]); setCurrentMatch(0) }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh' }}>
      ...
      <div style={{ padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.08)', background:'#141414', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
        ...
        <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', flexShrink:0, whiteSpace:'nowrap' }}>
          {isSearching ? 'Loading all pages for search…' : `${totalLines.toLocaleString()} lines${page < totalPages && !searchTerm ? ` · ${lines.length.toLocaleString()} loaded` : ''}`}
        </div>

        {/* Search bar */}
        <div style={{ display:'flex', alignItems:'center', gap:4, background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.15)', borderRadius:5, padding:'3px 7px', flexShrink:0 }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2.2">
            <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
          </svg>
          <input value={inputVal} onChange={e => handleSearchInput(e.target.value)} onKeyDown={onKeyDown}
            placeholder="Find…"
            style={{ width:180, fontSize:12, background:'none', border:'none', color:'rgba(255,255,255,0.9)', outline:'none', fontFamily:'Geist Mono,monospace' }} />
          {searchTerm && (
            <span style={{ fontSize:11, color: matchCount > 0 ? 'rgba(255,255,255,0.5)' : '#fc8181', whiteSpace:'nowrap', minWidth:55, textAlign:'right' }}>
              {isSearching ? '…' : matchCount > 0 ? `${currentMatch + 1}/${matchCount}` : 'No results'}
            </span>
          )}
          {matchCount > 0 && <>
            <button onClick={goPrev} title="Prev (Shift+Enter)" style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.5)', padding:'1px 3px', fontSize:13 }}>↑</button>
            <button onClick={goNext} title="Next (Enter)"      style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.5)', padding:'1px 3px', fontSize:13 }}>↓</button>
          </>}
          {inputVal && <button onClick={clearSearch} style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.3)', padding:'1px 3px', fontSize:14 }}>✕</button>}
        </div>
      </div>
